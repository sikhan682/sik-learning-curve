...
const MainGallery = () => {
		...
		try {
			if (!storedVideos) {
				const response = await axios(
					`https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=10&q=hernan%20cattaneo&key=${API_KEY}`
				);
	...
	return <section className='mainGallery'>{mainGalleryVideos}</section>;
};
